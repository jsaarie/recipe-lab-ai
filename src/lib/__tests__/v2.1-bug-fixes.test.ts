...
// ---------------------------------------------------------------------------
// Inline copies of the functions under test so we don't need path aliases
// (fractions.ts exports only pure functions — no React, no browser APIs)
// ---------------------------------------------------------------------------
...
function parseQuantity(qty: string): number | null {
  ...
  // Bug 1 fix: normalise "11/4" (digit(s) immediately before a fraction) → "1 1/4"
...
function formatQuantity(value: number): string {
  ...
  // Bug 2 fix: snap thirds/sixths before falling through to eighth-snapping
...
// ---------------------------------------------------------------------------
// Static source reads for Bugs 3, 4, 5
// ---------------------------------------------------------------------------
...
assert('parseQuantity("1 1/4") === 1.25 (space, no unicode)', parseQuantity("1 1/4"), 1.25);
...
// Keys that must appear in ML_PER_UNIT. Multi-word keys are always quoted strings
// in the object literal; single-word keys may appear as either bare identifiers
// (e.g. `pt: 473`) or quoted strings (e.g. `"pt": 473`).
// We search for the key followed by a colon (with optional space) and a digit.

function assertMLKey(key: string): void {
  // Matches both:  "pt": 473  AND  pt: 473  AND  "fl oz": 29
  const pattern = new RegExp(`["']?${key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']?\\s*:`);
  if (pattern.test(editorSrc)) {
    console.log(`  PASS  ML_PER_UNIT contains key "${key}"`);
    passed++;
  } else {
    const msg = `  FAIL  ML_PER_UNIT contains key "${key}"\n         pattern not found: ${pattern}`;
    console.error(msg);
    failures.push(msg);
    failed++;
  }
...
// ---------------------------------------------------------------------------
// Bug 4 — qtyDraft useEffect sync in recipe-card.tsx (static analysis)
// ---------------------------------------------------------------------------
...
// The fix: a useEffect that syncs qtyDraft when !editingQty and ingredient.quantity changes.
// We look for the useEffect body that sets qtyDraft and the dep array [ingredient.quantity, editingQty].
assertContains(
  "useEffect syncs qtyDraft when !editingQty",
  recipeCardSrc,
  "if (!editingQty) setQtyDraft(ingredient.quantity);",
);
assertContains(
  "useEffect dep array includes [ingredient.quantity, editingQty]",
  recipeCardSrc,
  "[ingredient.quantity, editingQty]",
);

// ---------------------------------------------------------------------------
// Bug 5 — parseQuantity used in patchedStepIngredients split case (static analysis)
// ---------------------------------------------------------------------------
...
assertContains(
  'parseQuantity imported from "@/lib/fractions"',
  labViewSrc,
  'import { parseQuantity, formatQuantity } from "@/lib/fractions"',
);

// Verify parseQuantity is used in the split case (patchedStepIngredients useMemo)
// The pattern is: parseQuantity(si.quantity) ?? parseFloat(si.quantity)
// This replaces a bare parseFloat(si.quantity)
assertContains(
  "parseQuantity used in patchedStepIngredients split case for si.quantity",
  labViewSrc,
  "parseQuantity(si.quantity) ?? parseFloat(si.quantity)",
);
assertContains(
  "parseQuantity used in patchedStepIngredients split case for si.totalQuantity",
  labViewSrc,
  "parseQuantity(si.totalQuantity) ?? parseFloat(si.totalQuantity)",
);
assertContains(
  "parseQuantity used in patchedStepIngredients split case for derived.quantity",
  labViewSrc,
  "parseQuantity(derived.quantity) ?? parseFloat(derived.quantity)",
);
